        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                while True:
                    ...
                    if decision == "all":
                        print(
                            "The test will be held for every {} port of the BrickPi3.".format(
                                port_type
                            )
                        )
    ...
    try:
        ...
        while True:
            ...
            if value:  # if the touch sensor is pressed
                ...
                ):  # if speed reached 100, start ramping down. If speed reached -100, start ramping up.
                    adder = -adder
                speed += adder
...
def motors_readings(intro: str, msg_start: str, test_function: Callable) -> None:
    ...
    try:
        while True:
            status = get_status_msg(msg_start, BP_MOTOR_PORTS, test_function)
            print(status)
...
"""
    ...
    def test_logic(
        port_number: int, other_ports: list[Port], other_ports_sum: int
    ) -> None:
...
"""
    ...
    def test_logic(
        ...
        while True:
            ...
            status = get_status_msg(
                f"Target: {target}   Motor position ", other_ports, BP.get_motor_encoder
            )
            print(status)
...
"""
    ...
    try:
        ...
        print(
            "Manufacturer    : ", get_brickpi3_value(BP.get_manufacturer)
        )  # read and display the serial number
        print(
            "Board           : ", get_brickpi3_value(BP.get_board)
        )  # read and display the serial number
        print(
            "Serial Number   : ", get_brickpi3_value(BP.get_id)
        )  # read and display the serial number
        print(
            "Hardware version: ", get_brickpi3_value(BP.get_version_hardware)
        )  # read and display the hardware version
        print(
            "Firmware version: ", get_brickpi3_value(BP.get_version_firmware)
        )  # read and display the firmware version
        print(
            "Battery voltage : ", get_brickpi3_value(BP.get_voltage_battery)
        )  # read and display the current battery voltage
        print(
            "9v voltage      : ", get_brickpi3_value(BP.get_voltage_9v)
        )  # read and display the current 9v regulator voltage
        print(
            "5v voltage      : ", get_brickpi3_value(BP.get_voltage_5v)
        )  # read and display the current 5v regulator voltage
        print(
            "3.3v voltage    : ", get_brickpi3_value(BP.get_voltage_3v3)
        )  # read and display the current 3.3v regulator voltage
...
"""
    ...
    try:
        while True:
            print(
                f"Battery voltage: {BP.get_voltage_battery():6.3f}  9v voltage: {BP.get_voltage_9v():6.3f}  5v voltage: {BP.get_voltage_5v():6.3f}  3.3v voltage: {BP.get_voltage_3v3():6.3f}"
            )
    ...
    try:
        while True:
            for i in range(101):  # count from 0-100
                ...
                time.sleep(
                    0.01
                )  # delay for 0.1 seconds (100ms) to reduce the Raspberry Pi CPU load and give time to see the LED pulsing.
